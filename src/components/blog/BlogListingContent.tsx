  ...
  return (
    <div className="flex flex-col" data-testid="blog-listing">
      ...
      <Section background="default" padding="sm" className="pt-0 min-h-[400px] pb-24">
        <Container>
          {showHeroFeatured && (
            <h2 className="text-2xl font-bold text-text-primary mb-8">Últimas publicaciones</h2>
          )}
          {!showHeroFeatured && !searchQuery && activeCategory === 'all' && (
            <h2 className="text-2xl font-bold text-text-primary mb-8">Todas las publicaciones</h2>
          )}
          ...
          {searchQuery && (
            <div className="mb-8">
              <h2 className="text-xl text-text-secondary">
                Se han encontrado{' '}
                <span className="font-bold text-text-primary">{filteredPosts.length}</span>{' '}
                {filteredPosts.length === 1 ? 'publicación' : 'publicaciones'} para "{searchQuery}"
              </h2>
            </div>
          )}
